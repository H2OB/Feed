...
-(void )celllayout{
    ...
    if (feed.content.length>0) {
        ...
        YYTextLayout *fiveLine=[self layout:CONTENTFONT color:CONTENTCOLOR width:kScreenWidth-2*NAMELEFT-AVTARX-AVTARW string:feed.content max:NO];
        YYTextLayout *maxLine=[self layout:CONTENTFONT color:CONTENTCOLOR width:kScreenWidth-2*NAMELEFT-AVTARX-AVTARW string:feed.content max:YES];
        ...
        if (maxLine.textBoundingSize.height==fiveLine.textBoundingSize.height) {
            ...
            _allBtnFrame=CGRectMake(CGRectGetMaxX(_avtarFrame)+NAMELEFT, self.cellHeight+5, 35, 20);
    ...
    if(feed.imageArr.count>0){
        ...
        NSInteger startX= CGRectGetMaxX(_avtarFrame)+NAMELEFT;
